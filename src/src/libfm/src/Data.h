...
class DataMetaInfo {
 public:
  ...
  void loadGroupsFromVector(std::vector<std::string> vmeta);
...
void DataMetaInfo::loadGroupsFromVector(std::vector<std::string> vmeta) {
  num_attr_groups = 0;
  
  std::vector<std::string>::iterator iter;
  for (iter=vmeta.begin(); iter<vmeta.end(); iter++) {
  //for (uint i = 0; i < attr_group.dim; i++) {
    num_attr_groups = std::max(num_attr_groups, uint(atoi((*iter).c_str()))+1);
  }
  num_attr_per_group.setSize(num_attr_groups);
  num_attr_per_group.init(0);
  for (iter=vmeta.begin(); iter<vmeta.end(); iter++) {
  //for (uint i = 0; i < attr_group.dim; i++) {
    num_attr_per_group(uint(atoi((*iter).c_str())))++;
  }
}
...
void Data::load(std::vector<std::string> vstr) {
  ...
  int load_from = 0;
  std::string filename = "off_function";
