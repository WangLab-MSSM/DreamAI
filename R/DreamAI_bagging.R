...
#'
#' @return list of imputed dataset, n.bag and a matrix containing gene name, sample name, true and imputed values of every pseudo missing combined from n.bag datasets. Impute is a list of imputed datasets (average over all pseudo imputed data matrices) by different methods as specified by the user. Always returns imputed data by "Ensemble"
#' @export
#' @examples
#' \dontrun{
#' data(datapnnl)
...
#' impute$Ensemble
#' }
DreamAI_Bagging<-function(data,k=10,maxiter_MF = 10, ntree = 100,maxnodes = NULL,maxiter_ADMIN=30,tol=10^(-2),gamma_ADMIN=NA,gamma=50,CV=FALSE,fillmethod="row_mean",maxiter_RegImpute=10,conv_nrmse = 1e-6,iter_SpectroFM=40,method=c("KNN","MissForest","ADMIN","Brinn","SpectroFM","RegImpute","Ensemble"),SamplesPerBatch,n.bag,save.out=TRUE,path=NULL,ProcessNum)
{
  ...
  for (i in 1:n.bag)
  {
    ...
    ResultDreamImputation<-DreamAI(data=data.pnnl.new,k=k,maxiter_MF = maxiter_MF, ntree = ntree,maxnodes = maxnodes,maxiter_ADMIN=maxiter_ADMIN,tol=tol,gamma_ADMIN=NA,gamma=gamma,CV=CV,fillmethod=fillmethod,maxiter_RegImpute=maxiter_RegImpute,conv_nrmse = conv_nrmse,iter_SpectroFM=iter_SpectroFM,method=method)
  ...
  bag.imputed_matrix<-list(imputed_matrix=list("KNN"=bag.knn,"MissForest"=bag.MissForest,"ADMIN"=bag.ADMIN,"Brinn"=bag.BruinGo,"SpectroFM"=bag.DMIS,"RegImpute"=bag.Jeremy,"Ensemble"=bag.ensemble),n.bag=n.bag,summary=summary.all1)
