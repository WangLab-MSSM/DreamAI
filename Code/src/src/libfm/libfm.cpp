...
// [[Rcpp::export]]
SEXP do_libfm(StringVector rcpp_data_table, StringVector arg_strs) {
  
  int argc = arg_strs.size();
  char* argv[arg_strs.size()];
  for(int i=0; i<arg_strs.size(); i++){
      argv[i] = arg_strs[i];
  }
  vector<string> data_table(rcpp_data_table.size());
  for(int i=0; i<rcpp_data_table.size(); i++){
    data_table[i] = rcpp_data_table[i];
  }
  ...
  try {
    ...
    // ret = ppp.preprocessor(cmdline.getValue(param_train_file));
    ret = ppp.preprocessor2(data_table);
    std::vector<std::string> train_vstr = ret[0];
    std::vector<std::string> test_vstr = ret[1];
    std::vector<std::string> meta_vint = ret[2];
    ...
    meta_main.loadGroupsFromVector(meta_vint);
    ...
    for(i=0; i<pred.dim; i++) {
        d_pred.push_back(pred(i));
    }
    
    std::vector<std::string> final_lines = ppp.postprocessor(d_pred);
    Rcpp::StringVector Rret(final_lines.size());
    
    for(i=0; i<final_lines.size(); i++){
        Rret[i] = final_lines[i];
    }
    
    //Rcpp::NumericVector Rret(pred.dim);
    //for(i=0; i<pred.dim; i++) {
    //    Rret[i] = pred(i);
    //}
    return Rret;
